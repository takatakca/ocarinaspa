-- Production-safe redemption of post-service Ocarina store credits.
-- A credit is first atomically reserved, then consumed only after the Stripe
-- invoice that contains the negative line item is finalized. This prevents
-- double use across concurrent admin requests.
...
BEGIN
  ...
  -- Idempotent retry: the same request may have reserved or already consumed the
  -- credit before the browser/network received the response. Return it to the
  -- same request only; a different request can never reuse it.
  RETURN QUERY
